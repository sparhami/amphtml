...
class AmpTwitter extends AMP.BaseElement {
  ...
  addDefaultPlaceholder_() {
    ...
    return this.element.appendChild(placeholder);
  }
  ...
  layoutCallback() {
    ...
    listenFor(iframe, MessageType.READY_TO_REQUEST_CONTENT, () => {
      this.updateForReadyState_();
    }, /* opt_is3P */true);
  ...
  /**
   * Updates when the tweet is ready to be requested.
   * @param {number} height The height of the rendered tweet.
   * @private
   */
  updateForReadyState_() {
    this.element.setAttribute('loading', 'ready');
...
AMP.extension('amp-twitter', '0.1', AMP => {
  AMP.registerElement('amp-twitter', AmpTwitter, CSS);
});
